<template>
	<q-page class="padding-page">
		<transition mode="out-in" enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
			<div v-if="$route.path == '/treinadores'" class="q-gutter-y-md" key="list">
				...
				<q-table class="sticky-header-table" :rows="perfis" :columns="perfisColumns" align="left" row-key="id"  @request="buscar" :rows-per-page-options="[5, 10, 25, 50, 100]" :pagination-label="paginationLabel" binary-state-sort>
					<template v-slot:body-cell-actions="props">
						<q-td :props="props">
							<q-btn icon="fas fa-edit"  color="primary" flat dense @click="editarCliente(props.row.id)" >
								<q-tooltip>Editar</q-tooltip>
							</q-btn>
							<q-btn icon="fas fa-eye"  color="primary" flat dense @click="showCliente(props.row.id)" >
								<q-tooltip>Mostrar</q-tooltip>
							</q-btn>
							<q-btn icon="fas fa-trash" color="primary"  flat dense @click="removerCliente(props.row.id)" >
