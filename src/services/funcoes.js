...
export const getCaminhoFoto = async function (path, foto) {
	let response = await this.metodoExecutar({ url: `api/${path}/imagem/${foto}`, method: 'get' })
...
export const getTenantLogo = async function () {
	// let response = await this.metodoExecutar({url:'api/Tenant/logo',method:'post'})
	//this.$store.commit('setDados', { key: 'tenantSrcLogo', value: response.status===200 ? response.data : ''})
}
...
export const metodoRespostaErro = function (response) {
	if (!response) this.$q.notify({ message: 'Não foi possível executar a solicitação!', type: 'negative' })
	else if (response.data.erros) this.$q.notify({ message: response.data.erros, type: 'negative', html: true })
	else if (response.data.error) this.$q.notify({ message: response.data.message ? response.data.message.toString() : response.data.error.e || '', type: 'negative', html: true })
...
export const formatarValorMilhar = function (v, moeda) {
	...
	return (moeda === undefined ? 'R$ ' : '') + (negativo ? '-' : '') + v2 + ',' + v1[1]
}
